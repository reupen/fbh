...
void sort_metadb_handle_list_by_format_get_permutation(TList&& p_list, mmh::Permutation& order,
    const service_ptr_t<titleformat_object>& p_script, titleformat_hook* p_hook, bool b_stablise = false)
{
    sort_metadb_handle_list_by_format_get_permutation_partial(
        p_list, order.get_count(), 0, order.get_count(), order, p_script, p_hook, b_stablise);
}

template <typename TList>
void sort_metadb_handle_list_by_format(TList&& p_list, const service_ptr_t<titleformat_object>& p_script,
    titleformat_hook* p_hook, bool b_stablise = false)
{
    mmh::Permutation perm(p_list.get_count());
    sort_metadb_handle_list_by_format_get_permutation_partial(
        p_list.get_ptr(), p_list.get_count(), 0, perm.get_count(), perm, p_script, p_hook, b_stablise);
